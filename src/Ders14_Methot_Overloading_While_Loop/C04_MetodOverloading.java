package Ders14_Methot_Overloading_While_Loop;

public class C04_MetodOverloading {
    public static void main(String[] args) {

        /*
            Java overloading kurallari
            1- İsim ve parametreleri ayni olna birden fazla
                method olusturmaz
            2- data turleri farkli olan parametrelerin yerlerini
            degistirirsek java yeni bir method olarak kabul eder.(int,double),
            (double,int)

                Overloading olan class larda
                Java hangi metodun calistiracagina su sekilde karar verir
                1- cagrilan isimde method var mi?
                2- method call da kullanilan arguments ile methodlardaki
                   parametrelerin %100 uyumlu oldugu var mı?
                 3- argument ile parametre ayni olmasa bile
                   verilen argumentleri kabul edecek parametre var mi?
                 4- Casting ile birden fazla methodu kullanabiliyorsa
                    daha az casting ile calisacak methodu kullanir
         */

        topla(3,5);
        topla(3.4,2.3);

        //topla(ali,veli); CTE bu argumentleri kabul edebilecek parametre yok

        topla('a', 'b'); // iki int 195
        topla(3f, 4f); // iki double 7.0

        topla(3f,4);
    }

    public static void topla (int a, int b){
        System.out.println("İki tamsayinin toplami : "+ (a+b));
    }

    public static void topla (double a, double b){
        System.out.println("İki double sayinin toplami: " + (a+b));
    }

    public static void topla (int sayi1, double sayi2) {
        System.out.println("int ve double sayilarin toplami: " + (sayi1 + sayi2));

    }
    public static void topla (double sayi1, int sayi2) {
        System.out.println("Double ve int sayilarin toplami: " + (sayi1 + sayi2));
    }
}
